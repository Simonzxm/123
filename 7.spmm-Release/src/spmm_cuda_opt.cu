									   ...
									   const float *vin, float *vout, int m, int n)
{
	...
	for (int row = warpGlobal; row < m; row += gridDim.x * warpsPerBlock)
	{
		...
		// Tile over dense dimension N in chunks of 64 columns per warp iteration
		for (int jt = 0; jt < n; jt += 64)
		{
			const int c0 = jt + lane;           // first column computed by this lane
			const int c1 = jt + lane + 32;      // second column computed by this lane
